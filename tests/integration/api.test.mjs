...
describe("Madinah Arabic API and static app", () => {
  ...
  it("serves the account page code and cache-busted assets", async () => {
    const page = await fetch(`${server.baseUrl}/`).then((response) => response.text());
    const app = await fetch(`${server.baseUrl}/app.js?v=20260620-question-flow-fix`).then((response) => response.text());
    const core = await fetch(`${server.baseUrl}/learning-core.js?v=20260620-question-flow-fix`).then((response) => response.text());
    ...
    assert.match(page, /20260620-question-flow-fix/);
